...
public readonly struct Circle : IEquatable<Circle>
{
    ...
    /// <summary>
    /// Returns a value that indicates whether the specified rectangle intersects with this circle.
    /// </summary>
    /// <param name="rectangle">The other rectangle to check.</param>
    /// <returns>true if the other rectangle intersects with this circle; otherwise, false.</returns>
    public bool Intersects(Rectangle rectangle)
    {
        int closestX;
        int closestY;

        if (Location.X < rectangle.Left) closestX = rectangle.Left;
        else if (Location.X > rectangle.Right) closestX = rectangle.Right;
        else closestX = Location.X;

        if (Location.Y < rectangle.Top) closestY = rectangle.Top;
        else if (Location.Y > rectangle.Bottom) closestY = rectangle.Bottom;
        else closestY = Location.Y;

        return Vector2.DistanceSquared(new Vector2(closestX, closestY), Location.ToVector2()) < Radius * Radius;
    }
