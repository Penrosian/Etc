...
namespace MonoGame_GameLibrary.Audio;
...
public class AudioController : IDisposable
{
    ...
    public float SongVolume
    {
        ...
        set
        {
            if (IsMuted)
            {
                _previousSongVolume = Math.Clamp(value, 0.0f, 1.0f);
            }
            else MediaPlayer.Volume = Math.Clamp(value, 0.0f, 1.0f);
        }
    }
